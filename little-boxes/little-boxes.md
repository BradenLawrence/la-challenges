...
If we modify this file, and add the `<Picture />` component somewhere inside the `return()` function, we should see it appear twice on the page &ndash; once where it did originally, and now also _inside_ the Wrapper component (which moves us one step closer to what our designers intended.)
...
Remember that we'll _also_ have to change our `main.js`, to give our component values for these props:
...
See if you can apply this same principle to refactor your new `Box` component so that it could replace _any_ of the `Wrapper`, `Picture`, `List`, or `Numbers` components. You'll need to use the same conditional logic as in the provided example (`if (props.paragraph) {...}`) to return different JSX elements depending on which `props` you provide.
